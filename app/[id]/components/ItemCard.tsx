          ...
          <p className='max-w-[90%]'>
            {item.product_name}
            {
              !done ?
                ...
                :
                <Badge variant="secondary" className={cn("text-sm absolute top-3 right-3 ml-auto  rounded-full",
        ...
        !done ?
          <div className="flex gap-2 items-center">
            <Input
              ...
              onChange={(e) => onQuantityChange(item.id, Math.min(Number.parseInt(e.target.value), item.demand_quantity))}
            ...
            <Button className="" variant="destructive" size={"icon"} onClick={() => onCancel(item)}>
              <XIcon className="w-6 h-6" />
            </Button>
          </div> :
          <div className="flex gap-2 items-center ml-auto">
            <Button size={"icon"} variant={"secondary"} className='border' onClick={() => onUncheck(item)}>
              <X className="w-6 h-6" />
            </Button>
          </div>
      }
