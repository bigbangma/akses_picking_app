    ...
    const fetchData = async () => {
      try {
        ...
        console.log(
          data.transfers
            .reverse()
            .map((t: Transfer) => ({ ...t, date: new Date(t.date) })),
        );
        const tranfersByDay = data.transfers.reduce(
          (acc: { [key: string]: Transfer[] }, transfer: Transfer) => {
                ...
                return (
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-1">
                    {Array.from(products.values()).map((item) => (
                      <div
                        key={item.id}
                        className="p-2 bg-white border rounded-xl flex gap-4"
                      >
                        <Image
                          src={
